...
def _create_certificate_custom_resoure(ctx, external_cert_data, external_tls_config, issuer, namespace, component_name, volumes, volume_mounts, deployment_description):
...
def deploy(ctx, namespace, component_name, image, replicas, always_pull, k8sconfig, **kwargs):
    ...
    try:
        ...
        if cmpv2_integration_enabled:
            ctx.logger.info("CMPv2 integration is enabled: " + str(cmpv2_integration_enabled))
        else:
            ctx.logger.info("CMPv2 integration is disabled: " + str(cmpv2_integration_enabled))
        # cert_manager_cpmv2_integration = cpmv2_integration_enabled
        cmpv2_integration_enabled = True
