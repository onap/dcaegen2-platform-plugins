...
def _create_projected_volume_with_password(cert_type, cert_secret_name, keystore_password_sec):
...
def _create_pem_projected_volume_with_password(cert_secret_name):
...
def _create_certificate_custom_resoure(ctx, external_cert_data,
    ...
    if cert_type != 'pem':
        custom_resource.get("spec")["keystores"] = _create_keystores_object(_get_keystores_object_type(cert_type), keystore_password_sec)
    ...
    if cert_type == 'pem':
        ...
        projected_volume_sources = _create_pem_projected_volume_with_password(cert_secret_name)
    else:
        ctx.logger.info("Creating volume with passwords")
        projected_volume_sources = _create_projected_volume_with_password(
            cert_type, cert_secret_name, keystore_password_sec)
