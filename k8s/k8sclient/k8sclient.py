...
def _create_certificate_custom_resoure(ctx, external_cert_data,external_tls_config, issuer, namespace, component_name, volumes, volume_mounts):
    ...
    ctx.logger.info("Namespace: " + namespace)
    ...
    #Todo use secret created before
    # password_secret_name = "password-secret-jks"
    password_secret_name, password_secret_key = create_secret_with_password(
        namespace, component_name + "-cert-password", 30)
    if cert_type != 'pem':
