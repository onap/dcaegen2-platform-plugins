...
def _add_truststore_merger_init_container(init_containers, volumes, tls_info, tls_config, external_cert, external_tls_config, truststore_merger_config):
    external_cert_output_path = external_cert.get("external_cert_directory")
    if not external_cert_output_path.endswith('/'):
       external_cert_output_path += '/'

    tls_cert_output_path = tls_info.get("cert_directory") or tls_config.get("component_cert_dir")
    if not tls_cert_output_path.endswith('/'):
       tls_cert_output_path += '/'

    env = {}
    env["TRUSTSTORES"] = external_cert_output_path
