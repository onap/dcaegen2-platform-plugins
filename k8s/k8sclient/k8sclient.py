...
def _create_certificate_custom_resoure(ctx, external_cert_data,
    ...
    secret_name = component_name + "-secret"
    ...
    ctx.logger.info("CRD: " + str_resource)
    keystore_password = "password-secret-jks"
    keystores = {"jks": {
        "create": True,
        "passwordSecretRef": {
    ...
    custom_resource.get("spec")["keystores"] = keystores
    str_resource = str(custom_resource)
    ...
    # Create the volumes and volume mounts
    items = []
    it = client.V1KeyToPath(key="keystore.jks", path="keystore.jks")
    items.append(it)
    sec = client.V1SecretVolumeSource(secret_name=secret_name, items=items)
    keystore_jks = sec.items
    # truststore_jks = client.V1Secret().data["truststore.jks"]
    # truststore_jks = sec.items[sec.items.index("truststore.jks")]

    ctx.logger.info("Single secret items")
    ctx.logger.info("keystore: " + str(keystore_jks) + "\n")
    ...
    # password_secret = client.V1SecretVolumeSource(secret_name=keystore_password)
