...
class DiscoveryClient(object):
    ...
    def _lazy_init():
        ...
        DiscoveryClient._consul_host = None
    ...
    @staticmethod
    def _safe_get(url):
        """safely http get from consul url"""
        try:
    ...
    @staticmethod
    def _get_from_consul(url_mask, key):
        """try several consul hosts until the first connected to http get to url"""
        if DiscoveryClient._consul_host:
            url = url_mask.format(consul_host=DiscoveryClient._consul_host, key=key)
            return url, DiscoveryClient._safe_get(url)

        for consul_host in DiscoveryClient._consul_hosts:
            DiscoveryClient._consul_host = consul_host
            url = url_mask.format(consul_host=DiscoveryClient._consul_host, key=key)
            response = DiscoveryClient._safe_get(url)
            if response:
                return url, response
    ...
    def get_service_url(service_name):
        ...
        url, response = DiscoveryClient._get_from_consul(DiscoveryClient.SERVICE_MASK, service_name)
    ...
    def get_value(key):
        ...
        url, response = DiscoveryClient._get_from_consul(DiscoveryClient.KV_MASK, key)
